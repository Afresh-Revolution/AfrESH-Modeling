...
import { sanitizeSearchQuery } from "@/lib/sanitizeSearchQuery";
...
export default function ApplicationsClient({
  ...
  const searchQuery = useMemo(
    () => sanitizeSearchQuery(searchRaw),
    [searchRaw]
  );

  const filtered = useMemo(
    () => applications.filter((row) => applicationMatchesSearch(row, searchQuery)),
    [applications, searchQuery]
  );

  const handleSearchChange = (value: string) => {
    setSearchRaw(sanitizeSearchQuery(value));
  };

  const handleDownload = () => {
    downloadApplicationsExcel(applications);
  };

  return (
    <>
      <div className={styles.applicationsToolbar}>
        <div className={styles.applicationsSearchWrap}>
          <label htmlFor="applications-search" className={styles.applicationsSearchLabel}>
            Search submissions
          </label>
          <input
            id="applications-search"
            type="search"
            className={styles.applicationsSearchInput}
      ...
      {searchQuery ? (
        <p className={styles.applicationsSearchMeta}>
          Showing {filtered.length} of {applications.length} submission
          {applications.length === 1 ? "" : "s"}
          {filtered.length === 0 ? " — try a different search" : ""}
        </p>
      ) : null}
      ...
      {applications.length === 0 ? (
        ...
        <p className={styles.adminSubtitle}>No submissions match your search.</p>
      ) : null}
    </>
  );
}
